...
- 함수를 정의할 때 사용하는 키워드는 def 입니다. (define)
- 정의해 놓은 함수를 사용하는 것을 호출(call) 이라고 부릅니다.
- 파이썬에서는 함수를 호출하려면 반드시 호출문보다 상단부에
함수를 먼저 정의해야 합니다. (객체 생성해서 호출하는 건 순서 상관 없음)
'''

# 함수의 정의 (1~x까지 누적합을 구하는 로직)
def calc_sum(end):
    sum = 0
...
'''
* 인수, 매개변수 (arguments)
- 인수(매개값)는 함수를 호출할 때 함수 실행에 필요한 값으로,
함수 호출 시 전달해 주는 값입니다. 
매개변수(parameter)는 그 인수를 받아내는 변수입니다. 
...
# return이 있는 함수는 다른 함수의 매개값으로도 사용이 가능합니다.
print(add(add(3,7),add(12,3)))
n = int(input('정수: '))
...
abc = multi(8, 6)
print(abc) # None (return 값이 없는 void 함수이기에 None이 출력된다)