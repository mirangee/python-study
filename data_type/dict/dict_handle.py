...
# Java map에서는 put을 이용해 대입하지만
# 파이썬은 다음과 같이 없는 키를 이용해 대입합니다.
eng_kor['banana'] = '바나나'  # 
print(eng_kor)
# {'student': '학생', 'apple': '사과', 'book': '책', 'banana': '바나나'}


# 사전에 데이터 수정하기
'''
사전 내부에 이미 존재하는 key를 사용하여 새로운 데이터를 대입하면,
해당 key 값에 매핑 되어있는 value가 수정됩니다.
'''
eng_kor['book'] = '서적'
print(eng_kor)
# {'student': '학생', 'apple': '사과', 'book': '서적', 'banana': '바나나'}
...
Java는 Map의 key를 따로 뽑아서 반복문을 돌렸지만 이런 차이가 있습니다.
'''
for k in eng_kor:
    print(f'{k} : {eng_kor[k]}')
